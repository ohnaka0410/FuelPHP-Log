...
{
	/**
	 * container for the Monolog instances
	 */
	protected static $monologs = array();
	 ...
	 * process id
	 */
	protected static $process_id = null;

	/**
	 * create the monolog instances
	 */
	public static function _init()
	{
		...
		for ($i = 0; $i < 10; $i++) {
			static::$process_id .= $str[mt_rand(0, strlen($str)-1)];
		}

		// create the monolog instances
		$logs = \Config::get('log', array('default'));
		if (empty($logs) || !is_array($logs))
		{
			$logs = array('default');
		}

		$is_array = (array_values($logs) === $logs);

		foreach ($logs as $key => $value)
		{
			$name   = $is_array ? $value  : $key;
			static::$monologs[$name] = new \Monolog\Logger($name);
	...
	{
		return \Arr::get(static::$monologs, $name, null);
	}

	/**
	 * initialize the created the monolog instance
	 */
	public static function initialize($name = 'default')
	{
		...
		$path = \Config::get('log.'.$name.'.path', APPPATH.'logs'.DS);

		// and make sure it exsts
		if ( ! is_dir($path) or ! is_writable($path))
		{
			\Config::set('log.'.$name.'.threshold', \Fuel::L_NONE);
			throw new \FuelException('Unable to create the log file. The configured '.$name.' log path "'.$path.'" does not exist.');
		}

		// determine the name of the logfile
		$filename = \Config::get('log.'.$name.'.file', null);
		if (empty($filename))
		{
			$filename = date('Y').DS.date('m').DS.date('d').DS.$name.'.php';
		}
		...
		{
			\Config::set('log.'.$name.'.threshold', \Fuel::L_NONE);
			throw new \FuelException('Unable to access the '.$name.' log file. Please check the permissions on '.$path.'. ('.$e->getMessage().')');
		}
		...
		$format      = \Config::get('log.'.$name.'.format', "%level_name% - %datetime% --> %message%".PHP_EOL);
		$format      = str_replace('%process_id%', static::$process_id, $format);
		$date_format = \Config::get('log.'.$name.'.date_format', 'Y-m-d H:i:s');
		$line_break  = \Config::get('log.'.$name.'.line_break', false);
		...
		$formatter = new \Monolog\Formatter\LineFormatter($format, $date_format, $line_break);
		$stream->setFormatter($formatter);
		static::instance($name)->pushHandler($stream);
	}
	 ...
	 */
	public static function info($msg, $context = null, $name = 'default')
	{
		return static::write(\Fuel::L_INFO, $msg, $context, $name);
	}
	 ...
	 * @return  bool    If it was successfully logged
	 */
	public static function debug($msg, $context = null, $name = 'default')
	{
		return static::write(\Fuel::L_DEBUG, $msg, $context, $name);
	}
	 ...
	 * @return  bool    If it was successfully logged
	 */
	public static function warning($msg, $context = null, $name = 'default')
	{
		return static::write(\Fuel::L_WARNING, $msg, $context, $name);
	}
	 ...
	 * @return  bool    If it was successfully logged
	 */
	public static function error($msg, $context = null, $name = 'default')
	{
		return static::write(\Fuel::L_ERROR, $msg, $context, $name);
	}
	 ...
	 */
	public static function log($level, $msg, array $context = array(), $name = 'default')
	{
		// bail out if we don't need logging at all
		if ( ! static::need_logging($level, $name))
		{
			return false;
		}
		...
		// log the message
		static::instance($name)->log($level, $msg, $context);
	 ...
	 * @throws	\FuelException
	 */
	public static function write($level, $msg, $context = null, $name = 'default')
	{
		// bail out if we don't need logging at all
		if ( ! static::need_logging($level, $name))
		{
		...
		// log the message
		empty($context) ? static::instance($name)->log($level, $msg) : static::instance($name)->log($level, $context.' - '.$msg);
	 ...
	 * @throws	\FuelException
	 */
	protected static function need_logging($level, $name = 'default')
	{
		...
		$loglabels = \Config::get('log.'.$name.'.threshold', \Fuel::L_NONE);
